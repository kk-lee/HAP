...
rr.sim.para <- read.csv("data_derived.v2/rr.sim.para.csv")
...
# histogram for India
india <- final %>% 
  filter(country=="India" & subgroup=="copd" & year==2017)

rr.sim <- exp(rnorm(num_iter, as.vector(india$rr.beta), india$rr.se))

pdf("results.v2/india.rr.hist.pdf",
    height=8,
    width=12)
hist((rr.sim), breaks=100, border="blue", main= "Histogram for Risk Ratio", xlab="Risk ratio", xlim=c(1.2, 2.4))
dev.off()


x <- rbeta(10000, india$alpha, india$beta)
x <- x*100

pdf("results.v2/india.prevalence.hist.pdf",
    height=8,
    width=12)
hist(x, breaks=100, border="blue", main= "Histogram for Prevalence", xlab="Prevalence, %")
dev.off()

y <- rnorm(10000,india$nm_mean,india$nm_se)
y <- exp(y)
y <- y/1000

pdf("results.v2/india.copd.burden.hist.pdf",
    height=8,
    width=12)
hist(y, breaks=100, border="blue", main= "Histogram for overall COPD DALYs", xlab="DALYs, thousands")
dev.off()

p <- ((x/100)*(rr.sim-1))/(1+(x/100)*(rr.sim-1))
p <- p*100

pdf("results.v2/india.paf.hist.pdf",
    height=8,
    width=12)
...
burden <- p*y/1000
pdf("results.v2/india.attr.burden.hist.pdf",
    height=8,
    width=12)
hist(burden, breaks=100, border="blue", main= "Histogram for COPD DALYs attributable to indoor air pollution", xlab="DALYs, thousands")
dev.off()
...
#### write csv
write_csv(final, "data_derived.v2/final.csv")
